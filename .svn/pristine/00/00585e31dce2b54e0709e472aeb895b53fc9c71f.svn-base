...
{
    public partial class MAGraeme : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                loadOpportunities("graeme");
            }
        }

        private void loadOpportunities(string username)
        {
            SqlCommand cmdMyQuery = new SqlCommand("SELECT Opportunities_ID FROM Opportunities WHERE Employee = '" + username + "' AND Opportunity_Stage = 'Opportunity' ORDER BY Opportunities_ID DESC");
        ...
        protected void lstOpportunities_SelectedIndexChanged(object sender, System.EventArgs e)
        {
            ...
            loadOpportunityDetails("graeme", selected_opp);
        }
